...
public class Robot {
	...
	if (this.vivant && autre.vivant && (autre != this) && enCollision) {
		...
		// Recenser les collisions dans le log
	...
	if (nbExplosions <= maxExplosions) {
	    ...
	    // Recenser les explosions dans le log
    ...
    /// TD5 : Message dependant de l'historique des logs.
    public void direBonjour(Robot autre) {

	/// Important : bloc try...catch
	try {
	    LogLine ligne = logCollisions.trouveLigne(this, autre) ;
	    System.out.println("Rebonjour, on s'est déjà croisé le " + ligne.getDate()) ;
	}
	catch (PasTrouve e) {
